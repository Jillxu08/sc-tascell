...
enum array { x11, x12, x21, x22, y11, y12, y21, y22,
    ...
    //第一个枚举成员的默认值为整型的 0，后续枚举成员的值在前一个成员上加 1. arrs=32
...
int main()
{
    ...
    for (i=0; i<N*N; i++)
    {
        // A[i] = rand()/(RAND_MAX);//+1 is removed
        // B[i] = rand()/(RAND_MAX);//+1 is removed
        A[i] = rand()/(RAND_MAX + 1.);
        B[i] = rand()/(RAND_MAX + 1.);
    }
...
void strassenMul(double *X, double *Y, double *Z, int m)    // X Y Z 均为double类型的指针
{
    int row = 0, col = 0;
    int n = m/2;
    int i = 0, j = 0;
    double *arr[arrs];    // each matrix mem ptr，指针数组，数组arr中的元素都为double型指针
