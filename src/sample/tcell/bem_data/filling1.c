// ============================================================
// filling.c - Multi-process + Single-GPU per process with MPI version
// ============================================================
...
static pthread_mutex_t gpu_mutex[4];  // 假设最多4个GPU
static int gpu_mutex_initialized = 0;

void initialize_gpu_mutexes() {
    if (!gpu_mutex_initialized) {
        int num_gpus = acc_get_num_devices(acc_device_nvidia);
        for (int i = 0; i < num_gpus && i < 4; i++) {
            pthread_mutex_init(&gpu_mutex[i], NULL);
        }
        gpu_mutex_initialized = 1;
    }
}


// 全局变量：存储分配的GPU ID
static int assigned_gpu_id = -1;
static int my_process_rank = -1;
...
void assign_gpu_to_process() {
    ...
    if (num_gpus == 0) {
        fprintf(stderr, "ERROR: No GPUs detected on process %d\n", my_process_rank);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    
    // 根据进程rank分配GPU
    assigned_gpu_id = my_process_rank % num_gpus;
    
    // 设置OpenACC默认设备
    acc_set_device_num(assigned_gpu_id, acc_device_nvidia);
    
    // 验证
    int current_device = acc_get_device_num(acc_device_nvidia);
    
    fprintf(stderr, "====== GPU Assignment ======\n");
    fprintf(stderr, "Process %d/%d: Assigned to GPU %d/%d\n",
            my_process_rank, num_procs, assigned_gpu_id, num_gpus);
    fprintf(stderr, "Process %d: Verified current GPU = %d\n",
            my_process_rank, current_device);
    fprintf(stderr, "============================\n");

    initialize_gpu_mutexes();  // 初始化锁
    fprintf(stderr, "Process %d: Initialized GPU mutex for GPU %d\n",
            my_process_rank, assigned_gpu_id);
}
...
void data_transfer(){
    // 确保在正确的GPU上
    if (assigned_gpu_id >= 0) {
        acc_set_device_num(assigned_gpu_id, acc_device_nvidia);
    }
    
    int current_gpu = acc_get_device_num(acc_device_nvidia);
    
    fprintf(stderr, "Process %d: Transferring data to GPU %d\n",
            my_process_rank, current_gpu);
    ...
    fprintf(stderr, "Process %d: Data transfer to GPU %d completed\n",
            my_process_rank, current_gpu);
}
              ...
              double* pa_ref, double* pb_ref, int* lrow_done, int* lcol_done, int nofc, double zgmid[][3], int f2n[][3], double bgmid[][3], int id, int use_gpu, double *lr_cpu_time, double *lr_gpu_time)
{
  ...
  if (use_gpu) {
    ...
    pthread_mutex_unlock(&gpu_mutex[assigned_gpu_id]);  
    t1 = get_time1();
    *lr_gpu_time += (t1 - t0);
...
void check_gpu_usage() {
    ...
    if (device_type == acc_device_nvidia) {
        printf("OpenACC is running on GPU %d\n", device_num);
    } else {
        printf("OpenACC is running on CPU \n");
    }
}
...
void fill_sub_leafmtx(struct leafmtx *st_lf, double znrmmat, int id, int *gpu_lr_cnt, int *cpu_lr_cnt, int *gpu_ds_cnt, int *cpu_ds_cnt, uint64_t *gpu_lr_elem, uint64_t *cpu_lr_elem, uint64_t *gpu_ds_elem, uint64_t *cpu_ds_elem, uint64_t *pen_ds_gpu_subm, uint64_t *pen_ds_gpu_elem, uint64_t *pen_lr_gpu_subm, uint64_t *pen_lr_gpu_elem, double *ds_cpu_time, double *lr_cpu_time, double *ds_gpu_time, double *lr_gpu_time){
  ...
  static int printed_workers[128] = {0};
  
  pthread_mutex_lock(&print_mutex);
  if (!printed_workers[id] && id < 128) {
      int my_rank;
      MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
      int gpu_id = acc_get_device_num(acc_device_nvidia);
      int num_gpus = acc_get_num_devices(acc_device_nvidia);
      
      fprintf(stderr, "[Init] Rank=%d Worker=%d: Sees %d GPUs, using GPU %d\n",
              my_rank, id, num_gpus, gpu_id);
      
      printed_workers[id] = 1;
  }
  ...
  if(ltmtx == 1){
    ...
    if((kparam * (ndl + ndt)) >= th_lr && assigned_gpu_id >= 0 &&
      pthread_mutex_trylock(&gpu_mutex[assigned_gpu_id]) == 0){
      use_gpu = 1;
      (*gpu_lr_cnt)++;
    } else {
      (*cpu_lr_cnt)++;
    ...
    if (can_use_gpu && assigned_gpu_id >= 0) {
        (*pen_ds_gpu_subm)++;
        (*pen_ds_gpu_elem) += ns;

        // if (pthread_mutex_trylock(&myMutex) == 0) {
        if (pthread_mutex_trylock(&gpu_mutex[assigned_gpu_id]) == 0) {
            got_gpu = 1;
        }
    }
    ...
    #pragma acc data if(use_gpu) copy(st_lf->a1[0:ns])
    {
      ...
      if (use_gpu) {
        ...
        int my_rank;
        MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
        int gpu_now = acc_get_device_num(acc_device_nvidia);
        
        static int print_count = 0;
        if (print_count < 10) {
            fprintf(stderr, "[DS GPU Check] Rank=%d Worker=%d: Using GPU %d (assigned=%d) %s\n",
                    my_rank, id, gpu_now, assigned_gpu_id,
                    (gpu_now == assigned_gpu_id) ? "✅" : "❌ ERROR!");
            print_count++;
        }}
        // ==============================
