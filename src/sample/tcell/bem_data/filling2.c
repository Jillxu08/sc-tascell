// ============================================================
// filling.c - Single-process + Multi-GPU per process with MPI version
// ============================================================
...
// GPU统计信息（动态分配）
typedef struct {
    pthread_mutex_t mutex;  // 每个GPU独立锁，减少竞争
    int task_count;
    int ds_count;
    int lr_count;
    uint64_t total_size;
...
void initialize_multi_gpu() {
    ...
    if (!multi_gpu_initialized) {
        ...
        for (int i = 0; i < num_gpus; i++) {
            ...
            gpu_data_ready[i] = 0;
            gpu_stats[i].task_count = 0;
            gpu_stats[i].ds_count = 0;
            gpu_stats[i].lr_count = 0;
            gpu_stats[i].total_size = 0;
            gpu_stats[i].ds_size = 0;
            gpu_stats[i].lr_size = 0;
            gpu_stats[i].total_time = 0.0;
        }
        ...
        if (!gpu_record_vec.records) {
            fprintf(stderr, "WARNING: Process %d failed to allocate record buffer, "
                          "only statistics will be available\n", my_rank);
        }
        ...
        fprintf(stderr, "\n");
        fprintf(stderr, "╔════════════════════════════════════════════════════╗\n");
        fprintf(stderr, "║   Multi-GPU Dynamic Allocation (Process %d)        ║\n", my_rank);
        fprintf(stderr, "╠════════════════════════════════════════════════════╣\n");
        fprintf(stderr, "║  Detected GPUs: %d                                  ║\n", num_gpus);
        ...
        fprintf(stderr, "║  Thresholds: DS=%d, LR=%d                        ║\n", th_ds, th_lr);
        fprintf(stderr, "╚════════════════════════════════════════════════════╝\n");
        fprintf(stderr, "\n");
    }
...
void transfer_data_to_all_gpus() {
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "╔════════════════════════════════════════════════════╗\n");
    fprintf(stderr, "║        Transferring Data to All GPUs              ║\n");
    fprintf(stderr, "╚════════════════════════════════════════════════════╝\n");
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "Process %d: All %d GPUs initialized and ready\n", my_rank, num_gpus);
    fprintf(stderr, "\n");
}
    ...
    if (gpu_record_vec.records != NULL) {
        ...
        if (gpu_record_vec.count >= gpu_record_vec.capacity) {
            ...
            if (new_records) {
                ...
                if (expand_count < 3) {  // 只打印前3次扩容
                    fprintf(stderr, "[Record] Expanded capacity to %d (%.2f MB)\n",
                            new_capacity,
                            new_capacity * sizeof(gpu_usage_record_t) / (1024.0 * 1024.0));
                    expand_count++;
                }
            } else {
                fprintf(stderr, "WARNING: Failed to expand record buffer at %d\n",
                        gpu_record_vec.capacity);
...
void print_gpu_usage_report() {
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "╔════════════════════════════════════════════════════════════════════════╗\n");
    fprintf(stderr, "║            GPU Usage Statistics (Process %d)                           ║\n", my_rank);
    fprintf(stderr, "╠════════════════════════════════════════════════════════════════════════╣\n");
    ...
    if (num_gpus > 1) {
        ...
        fprintf(stderr, "  Max tasks: %d, Min tasks: %d\n", max_tasks, min_tasks);
        fprintf(stderr, "  Balance ratio: %.2f ", balance);
        
        if (balance < 1.2) {
            fprintf(stderr, "✅ Excellent (very well balanced)\n");
        } else if (balance < 1.5) {
            fprintf(stderr, "✓ Good (acceptably balanced)\n");
        } else if (balance < 2.0) {
            fprintf(stderr, "⚠ Fair (some imbalance)\n");
        } else {
            fprintf(stderr, "❌ Poor (significant imbalance)\n");
        }
    }
    ...
    if (gpu_record_vec.records != NULL) {
        pthread_mutex_lock(&gpu_record_vec.mutex);
        fprintf(stderr, "\nDetailed Records: %d / %d (%.1f%% utilized)\n",
                gpu_record_vec.count,
                gpu_record_vec.capacity,
                100.0 * gpu_record_vec.count / gpu_record_vec.capacity);
        pthread_mutex_unlock(&gpu_record_vec.mutex);
    }
