...
// ========== 辅助函数（需要在其他地方实现）==========
void comp_row(double* zaa, double* zab, int ndl, int ndt, int k, int il,
              double* row, int nstrtl, int nstrtt, int* lrow_done, double* zau,
              int nofc, double zgmid[][3], int f2n[][3], double bgmid[][3],
              int use_gpu, int id, double *lr_cpu_time, double *lr_gpu_time);

void comp_col(double* zaa, double* zab, int ndl, int ndt, int k, int jt,
              double* col, int nstrtl, int nstrtt, int* lcol_done, double* zau,
              int nofc, double zgmid[][3], int f2n[][3], double bgmid[][3],
              int use_gpu, int id, double *lr_cpu_time, double *lr_gpu_time);
...
void data_transfer();  // 兼容原有接口
int try_acquire_any_gpu(int worker_id);
void release_gpu(int gpu_id);

// ========== GPU使用统计 ==========
void record_gpu_usage(int worker_id, int gpu_id, uint64_t size, double duration, const char* type);
