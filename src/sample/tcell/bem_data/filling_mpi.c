// haku CPU-only with MPI version
#include "filling.h"
...
void fill_sub_leafmtx(struct leafmtx *st_lf, double znrmmat){
  ...
  if(ltmtx == 1){
    // printf("Filling low-rank matrix: nstrtl=%d, ndl=%d, nstrtt=%d, ndt=%d\n", nstrtl, ndl, nstrtt, ndt);
    ...
    // printf("Filling full matrix: nstrtl=%d, ndl=%d, nstrtt=%d, ndt=%d\n", nstrtl, ndl, nstrtt, ndt);
